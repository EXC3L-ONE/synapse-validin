...
if not os.getenv("VALIDIN_API_KEY"):
    raise ValueError(
        "VALIDIN_API_KEY environment variable is not set. Please check your .env file."
    )

# Color constants for consistent styling
CYAN = "\033[96m"  # Section headers
BLUE = "\033[94m"  # Test stage headers
YELLOW = "\033[93m"  # Informational messages
GREEN = "\033[92m"  # Success messages
MAGENTA = "\033[95m"  # Test targets (IPs, domains)
...
class AcmeValidinTest(s_test.StormPkgTest):
    ...
    async def _setup_validin(self, core, enterprise=False):
        ...
        # Set hostname first
        hostname_cmd = "ex.validin.setup.hostname --self"
        if enterprise:
            hostname_cmd += " --enterprise"
        msgs = await core.stormlist(f"{hostname_cmd} --debug")
    ...
    async def test_validin_setup(self):
        ...
        async with self.getTestCore() as core:
            print(f"\n{CYAN}=== Starting Validin Setup Testing ==={RESET}")

            # Test setting hostname for community
            print(f"\n{BLUE}Testing Community Hostname Configuration{RESET}")
            msgs = await core.stormlist("ex.validin.setup.hostname --self --debug")
            self.stormIsInPrint("Setting Validin hostname for the current user", msgs)
            self.stormHasNoWarnErr(msgs)
            print(
                f"{YELLOW}Successfully set community hostname to:{RESET} {MAGENTA}app.validin.com{RESET}"
            )

            # Test setting hostname for enterprise
            print(f"\n{BLUE}Testing Enterprise Hostname Configuration{RESET}")
            msgs = await core.stormlist(
                "ex.validin.setup.hostname --enterprise --self --debug"
            )
            self.stormIsInPrint("Using enterprise hostname: pilot.validin.com", msgs)
            self.stormHasNoWarnErr(msgs)
            print(
    ...
    async def test_pdns(self):
        ...
        async with self.getTestCore() as core:
            ...
            # where value is fqdn and key is the IP we queried
            ...
            self.ge(len(nodes), 0)  # Using ge since AAAA records might not always exist
            print(
